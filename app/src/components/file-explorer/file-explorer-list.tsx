...
// ponytail: ContextMenuTrigger can't render as <tr>, so we wrap
// each row in a display:contents span to keep table layout intact
      ...
      <TableBody>
        {folders.map((folder) => {
          ...
          return (
            ...
            >
              <TableRow className="cursor-pointer" onClick={() => onFolderClick(folder.id)}>
                <TableCell>
        ...
        {documents.map((doc) => (
          ...
          >
            <TableRow className="cursor-pointer" onClick={() => onDocumentClick(doc.id)}>
              <TableCell>
                <div className="flex items-center gap-2">
